...
true_data = data_import.data_import_2014()  # 填你自己的数据地址

target = 'mf'

# 这里加一些数据的预处理, 最后需要的格式是pd.series

true_data = np.array(true_data['mf'])
...
if Train:
    ...
    # 创建折线图
    y_true = np.array([i[0] for i in reals])
    plt.plot(y_true, label='true')  # 实际值
    plt.plot(results, label='pred')  # 预测值

    print(y_true)
    print(results)

    mse = np.mean((y_true - results) ** 2) * (test_data.max() - test_data.min()) ** 2
    rmse = np.sqrt(mse)
    mape = np.mean(np.abs((y_true - results) / y_true)) * 100
    print('MSE:', mse)
    print('RMSE:', rmse)
    print('MAPE:', mape)
    # 增强视觉效果
